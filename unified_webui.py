...
/* Sidebar Navigation */
.sidebar {
    @apply fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-slate-900 to-slate-800 border-r border-slate-700 shadow-xl z-50;
}
...
.sidebar-logo {
    @apply text-white font-bold text-2xl tracking-tight;
}
...
.nav-item {
    @apply flex items-center px-6 py-3 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-slate-700 hover:to-slate-600 transition-all duration-300 cursor-pointer border-l-4 border-transparent;
}

.nav-item.active {
    @apply text-white bg-gradient-to-r from-blue-600 to-purple-600 border-l-4 border-white/30 shadow-lg;
}
...
/* Tab Navigation - Hidden for Dashboard Style */
.tab-nav {
...
.gr-textbox, .gr-dropdown, .gr-audio {
    @apply rounded-lg border-2 border-slate-600 bg-slate-700/50 text-white transition-all duration-300 shadow-lg hover:border-slate-500 focus:border-blue-400 focus:ring-4 focus:ring-blue-400/20;
}

.gr-textbox:focus, .gr-dropdown:focus, .gr-audio:focus {
    @apply border-blue-400 ring-4 ring-blue-400/20 transform -translate-y-px;
}

.gr-textbox {
    @apply text-base bg-slate-700/80 border-slate-600;
}
...
.gr-audio .gr-audio-player {
    @apply bg-slate-800 rounded-lg border border-slate-600;
}
...
.gr-slider input[type="range"] {
    @apply h-2 rounded-lg bg-gradient-to-r from-slate-600 to-slate-500;
}

/* Button Enhancements - Neon Style */
.gr-button-primary {
    @apply bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 hover:from-blue-500 hover:via-purple-500 hover:to-pink-500 text-white font-bold py-3 px-6 rounded-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl shadow-lg text-sm uppercase tracking-wide border border-white/20;
}
...
.gr-button-secondary {
    @apply bg-gradient-to-r from-slate-700 to-slate-600 border-2 border-slate-500 text-slate-100 font-semibold py-3 px-6 rounded-lg transition-all duration-300 hover:from-slate-600 hover:to-slate-500 hover:-translate-y-1 hover:border-slate-400;
}

/* Event Tags Styling - Neon Accents */
.tag-container {
    @apply flex flex-wrap gap-3 mt-4 mb-6 border-0 bg-transparent p-0;
}

.tag-btn {
    @apply min-w-fit w-auto h-10 text-sm bg-gradient-to-r from-slate-700 to-slate-600 border-2 border-slate-500 text-slate-100 rounded-full px-4 m-0 shadow-lg transition-all duration-300 font-semibold hover:from-slate-600 hover:to-slate-500 hover:-translate-y-1 hover:scale-105 hover:border-slate-400 hover:shadow-xl;
}

/* Accordion Enhancements */
.gr-accordion {
    @apply rounded-lg border-2 border-slate-600 bg-gradient-to-r from-slate-800 to-slate-700 shadow-xl;
}
...
.gr-info {
    @apply bg-gradient-to-r from-blue-900/50 to-blue-800/50 border-2 border-blue-500/50 text-blue-200 rounded-lg p-4 font-semibold shadow-lg;
}

.audio-note {
    @apply bg-gradient-to-r from-amber-900/50 to-amber-800/50 border-2 border-amber-500/50 rounded-lg p-4 text-amber-200 font-semibold shadow-lg;
}

/* Success and Error States */
.gr-success {
    @apply bg-gradient-to-r from-green-900/50 to-green-800/50 border-2 border-green-500/50 text-green-200 rounded-lg p-4 font-semibold shadow-lg;
}

.gr-error {
    @apply bg-gradient-to-r from-red-900/50 to-red-800/50 border-2 border-red-500/50 text-red-200 rounded-lg p-4 font-semibold shadow-lg;
}

/* Progress Bar - Neon Style */
.gr-progress-bar {
    @apply h-3 rounded-lg bg-slate-700;
}
...
@media (max-width: 768px) {
    .sidebar {
        @apply fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-slate-900 to-slate-800 border-r border-slate-700 shadow-xl z-50 transform -translate-x-full transition-transform duration-300;
    }
...
.dashboard-header {
    @apply text-white font-bold text-3xl mb-6 bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent;
}

.section-title {
    @apply text-white font-semibold text-xl mb-4 border-l-4 border-blue-500 pl-4;
}
...
with gr.Blocks(title="Chatterbox Unified WebUI", css=CUSTOM_CSS) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🎤 Chatterbox TTS"):
            with gr.Row():
                with gr.Column():
                    tts_text = gr.Textbox(
                    ...
                    tts_ref_wav = gr.Audio(sources=["upload", "microphone"], type="filepath", label="Reference Audio File", value=get_sample_audio_path("prompts/female_random_podcast.wav"))
                    tts_exaggeration = gr.Slider(0.25, 2, step=.05, label="Exaggeration (Neutral = 0.5, extreme values can be unstable)", value=.5)
                    tts_cfg_weight = gr.Slider(0.0, 1, step=.05, label="CFG/Pace", value=0.5)
                    ...
                    with gr.Accordion("More options", open=False):
                        ...
                        tts_min_p = gr.Slider(0.00, 1.00, step=0.01, label="min_p || Newer Sampler. Recommend 0.02 > 0.1. Handles Higher Temperatures better. 0.00 Disables", value=0.05)
                        tts_top_p = gr.Slider(0.00, 1.00, step=0.01, label="top_p || Original Sampler. 1.0 Disables(recommended). Original 0.8", value=1.00)
                        tts_repetition_penalty = gr.Slider(1.00, 2.00, step=0.1, label="repetition_penalty", value=1.2)
        ...
        with gr.TabItem("Chatterbox Turbo"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    turbo_run_btn = gr.Button("Generate ⚡", variant="primary")

                with gr.Column():
                    turbo_audio_output = gr.Audio(label="Output Audio")

                    with gr.Accordion("Advanced Options", open=False):
                        turbo_seed_num = gr.Number(value=0, label="Random seed (0 for random)")
                        turbo_temp = gr.Slider(0.05, 2.0, step=.05, label="Temperature", value=0.8)
                        ...
                        turbo_repetition_penalty = gr.Slider(1.00, 2.00, step=0.05, label="Repetition Penalty", value=1.2)
                        turbo_min_p = gr.Slider(0.00, 1.00, step=0.01, label="Min P (Set to 0 to disable)", value=0.00)
                        turbo_norm_loudness = gr.Checkbox(value=True, label="Normalize Loudness (-27 LUFS)")
        ...
        with gr.TabItem("Multilingual TTS"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Accordion("More options", open=False):
                        mtl_seed_num = gr.Number(value=0, label="Random seed (0 for random)")
                        mtl_temp = gr.Slider(0.05, 5, step=.05, label="Temperature", value=.8)

                    mtl_run_btn = gr.Button("Generate", variant="primary")

                with gr.Column():
                    mtl_audio_output = gr.Audio(label="Output Audio")
